...
# 1.
class(covid19.mn$date)

# 2.
covid19.mn <- subset(covid19, state=="Minnesota")

# 3.
covid19.mn$month <- month(covid19.mn$date)
covid19.mn$mday <- mday(covid19.mn$date)
covid19.mn$weekday <- factor(weekdays(covid19.mn$date),
                             levels=c("Monday", "Tuesday", "Wednesday", 
                                      "Thursday", "Friday",
                                      "Saturday", "Sunday"))

# 4.
cov.mon <- sort(unique(covid19.mn$month))
pos.inc <- vector()
for (i in cov.mon){
  pos.inc[i-min(cov.mon)+1] <- mean(subset(covid19.mn, month==i)$positive_increase)
}
print(pos.inc)

# 5.
ggplot(covid19.mn, aes(x=date, y=positive_increase, color=weekday)) +
  geom_point()
#peaks are mondays, valleys are saturdays and sundays
